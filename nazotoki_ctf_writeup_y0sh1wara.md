...
## 今回CTFの感想
馴染みのある誕生日と対応した黄道12星座を辿っていく冒険感があってとてもエキサイティングでした．
参加人数が多いのか，それともへびつかい座でDDOS攻撃をする方がいらしたのかでサーバが落ちて大変そうだなと思いました．
...
解答：
`テントウ` をSubmitする．
「〇〇〇〇ムシ」といったら天道虫くらいが思い付いたので提出したらクリアした．
...
解答：
分からんので「Webアプリケーションのセキュリティ分野の研究・ガイドラインの作成・脆弱性診断ツールの開発・イベント開催などの活動をしているオープンソースソフトウェアコミュニティの名称の読み方をカタカナ4文字で答えよ。」でググった．<br>
...
つまり，x = b^p ⇔ log_b(x) = p log_b(b) = pである． 
...
https://kotobank.jp/word/riddle-1245112
...
「★」マークはテキストとは限りませんとのことから，クリックするとai-intro.pngとしてダウンロードできる画像に「★」マークを上下に確認した．
主人公の高橋ピヨ彦くんは"セキュリティ業界の星「★」"を目指しているので，その上下の「★」マークにある文字を読むと`ハン ド          ル`とある．`ハンドル`をSubmitした．
...
User Agentの項目で，Select automaticallyのチェックを外して，Customを選択して，`stardustChrome`を入力して，Webページを更新した．
...
ということで，社員ナンバーに「9999」，パスワードに「' or 1=1 ;」を入力すると，社員ナンバー9999の社員情報が出てきて，attribute メモに以下のメッセージが確認できた．
...
プールの水面を撮影したようなpisces.jpgだが．．．両目立体視，立体写真とも呼ばれ，「交差法」（寄り目で見る）または「並行法」という見方で見ると文字が浮かび上がる．
...
強調されている1文字目を取り出すと，「ハテナイチオクカイタタケ」とある．
よって，DDOS攻撃のようにWebページ左上の?アイコンを1億回クリック（アクセス）する方法を取ろうとした方もいらっしゃるでしょうが，1億回は教義時間以内に叩けないことや，サーバーダウンの原因になるので違う方法を考える．
...
かに座のファイルたちはpcapファイルだったのでWiresharkで中身を見た．どうやらFTP通信しているようで，Protocolカラムを見るとFTP-DATAのやり取りをしているところがあるので，FTP Data(1009 bytes data)を右クリックしてShow Packet Bytesを選択すると画像を
すると，ファイルごとにさそり座やおひつじ座．．．などが見られる．
...
pcapファイルの分割・結合
commandでも
https://giugno.hatenadiary.org/entry/20110914/1315983399
ドラッグ＆ドロップでも出来る．
https://sugarsugar.conf.jp/wireshark%E3%81%A7%E3%81%AE%E3%82%AD%E3%83%A3%E3%83%97%E3%83%81%E3%83%A3%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB%E3%81%AE%E3%83%9E%E3%83%BC%E3%82%B8%E6%96%B9%E6%B3%95
